...
import org.apache.spark.sql.catalyst.expressions.{Attribute, Expression, MutableProjection, PythonUDTF}
  ...
  def createEvaluator(
    ...
    new ArrowPythonUDTFEvaluator {
      override def evaluate(
        ...
        val argOffsets = pythonUDTF.children.map {
          e =>
        ...
        val projectedRowIter = new ContextAwareIterator(context, iter).map(projection)
        val batchIter =
