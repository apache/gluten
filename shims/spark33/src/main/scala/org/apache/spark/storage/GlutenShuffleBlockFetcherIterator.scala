    ...
    while (result == null) {
      ...
      result match {
        ...
        case PushMergedLocalMetaFetchResult(
          ...
          try {
            val bufs: Seq[ManagedBuffer] = blockManager.getLocalMergedBlockData(
              shuffleBlockId,
              localDirs)
...
private class GlutenBufferReleasingInputStream(
   ...
   * the `read` and `skip` methods inside `BufferReleasingInputStream` currently.
   */
  private def tryOrFetchFailedException[T](block: => T): T = {
    try {
      ...
      case e: IOException if detectCorruption =>
        val diagnosisResponse =
          checkedInOpt.map(checkedIn => iterator.diagnoseCorruption(checkedIn, address, blockId))
