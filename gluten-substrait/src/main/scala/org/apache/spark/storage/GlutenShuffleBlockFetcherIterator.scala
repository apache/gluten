 ...
 *
 * This creates an iterator of (BlockID, InputStream) tuples so the caller can handle blocks in a
 ...
 *   list of blocks to fetch grouped by the [[BlockManagerId]]. For each block we also require two
 *   info: 1. the size (in bytes as a long field) in order to throttle the memory usage; 2. the
 *   mapIndex for this block, which indicate the index in the map stage. Note that zero-sized blocks
 *   are already excluded, which happened in
 *   [[org.apache.spark.MapOutputTracker.convertMapStatuses]].
 * @param mapOutputTracker
 *   [[MapOutputTracker]] for falling back to fetching the original blocks if we fail to fetch
 ...
 *   max number of shuffle blocks being fetched at any given point for a given remote host:port.
 ...
 *   whether the shuffle checksum is enabled. When enabled, Spark will try to diagnose the cause of
 *   the block corruption.
 * @param checksumAlgorithm
 *   the checksum algorithm that is used when calculating the checksum value for the block data.
...
final private[spark] class GlutenShuffleBlockFetcherIterator(
  ...
  /**
   * Count the retry times for the blocks due to Netty OOM. The block will stop retry if retry times
   * has exceeded the [[maxAttemptsOnNettyOOM]].
   */
  private[this] val blockOOMRetryCounts = new HashMap[String, Int]

  /**
   * The blocks that can't be decompressed successfully, it is used to guarantee that we retry at
  ...
  private[this] val onCompleteCallback = new GlutenShuffleFetchCompletionListener(this)

  private[this] val pushBasedFetchHelper =
    new GlutenPushBasedFetchHelper(
      this,
      shuffleClient,
      blockManager,
      mapOutputTracker,
  ...
  def cleanup(): Unit = {
    ...
    val threadIds = currentResults.keys()
    while (threadIds.hasMoreElements) {
      val threadId = threadIds.nextElement()
      val result = currentResults.remove(threadId)
      if (result != null) {
        result.buf.release()
      }
  ...
  private[this] def sendRequest(req: FetchRequest): Unit = {
    logDebug(
      "Sending request for %d blocks (%s) from %s"
        .format(req.blocks.size, Utils.bytesToString(req.size), req.address.hostPort))
  ...
  private def createFetchRequest(
    ...
    logDebug(
      s"Creating fetch request of ${blocks.map(_.size).sum} at $address "
   ...
   * `ManagedBuffer`'s memory is allocated lazily when we create the input stream, so all we track
  ...
  private[this] def fetchHostLocalBlocks(
    ...
    if (hostLocalBlocksWithMissingDirs.nonEmpty) {
      logDebug(
        s"Asynchronous fetching host-local blocks without cached executors' dir: " +
    ...
    if (hostLocalBlocksWithCachedDirs.nonEmpty) {
      logDebug(
        s"Synchronous fetching host-local blocks with cached executors' dir: " +
  ...
  private def fetchMultipleHostLocalBlocks(
    ...
    if (allFetchSucceeded) {
      logDebug(
        s"Got host-local blocks from ${bmIdToBlocks.keys.mkString(", ")} " +
  ...
  private def shuffleMetricsUpdate(blockId: BlockId, buf: ManagedBuffer, local: Boolean): Unit = {
    if (local) {
  ...
  /**
   * Fetches the next (BlockId, InputStream). If a task fails, the ManagedBuffers underlying each
   * InputStream will be freed by the cleanup() method registered with the TaskCompletionListener.
   * However, callers should close() these InputStreams as soon as they are no longer needed, in
  ...
  override def next(): (BlockId, InputStream) = {
    ...
    while (result == null) {
      ...
      result match {
          ...
          try {
            ...
            bufs.zipWithIndex.foreach {
              case (buf, chunkId) =>
                buf.retain()
                val shuffleChunkId =
                  ShuffleBlockChunkId(shuffleId, shuffleMergeId, reduceId, chunkId)
        ...
        case PushMergedRemoteMetaFailedFetchResult(shuffleId, shuffleMergeId, reduceId, address) =>
  ...
  /**
   * Get the suspect corruption cause for the corrupted block. It should be only invoked when
   * checksum is enabled and corruption was detected at least once.
   *
   * This will firstly consume the rest of stream of the corrupted block to calculate the checksum
   * of the block. Then, it will raise a synchronized RPC call along with the checksum to ask the
   * server(where the corrupted block is fetched from) to diagnose the cause of corruption and
   * return it.
   *
   ...
   * failure related to a push-merged block or shuffle chunk. This is executed by the task thread
   ...
   * the current chunk that had a fetch failure. This is executed by the task thread when the
  ...
  private[storage] def removePendingChunks(
    ...
    deferredFetchRequests.get(address).foreach {
      defRequests =>
...
private class GlutenBufferReleasingInputStream(
   ...
   * IOException as FetchFailedException when detectCorruption is true. This method is only used by
   * the `available`, `read` and `skip` methods inside `BufferReleasingInputStream` currently.
   */
  private def tryOrFetchFailedException[T](block: => T): T = {
    try {
      ...
      case e: IOException if detectCorruption =>
        val diagnosisResponse = checkedInOpt.map {
          checkedIn => iterator.diagnoseCorruption(checkedIn, address, blockId)
        }
 ...
 * @param data
 *   the ShuffleBlockFetcherIterator to process
 */
private class GlutenShuffleFetchCompletionListener(var data: GlutenShuffleBlockFetcherIterator)
