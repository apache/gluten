...
function apply_enhanced_velox_compatibility_patches {
  ...
  if [[ "$ENABLE_ENHANCED_FEATURES" == "ON" ]] &&
     ! grep -Fq "kWriterRowGroupSizeSession" \
       "$VELOX_HOME/velox/dwio/parquet/common/ParquetConfig.h"; then
    pushd "$VELOX_HOME"
    git apply --check "$CURRENT_DIR/parquet_writer_row_group_size.patch"
    git apply "$CURRENT_DIR/parquet_writer_row_group_size.patch"
    popd
  fi
}
