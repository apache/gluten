...
import org.apache.gluten.extension.OffloadDeltaDmlScan
...
class VeloxDelta24WriteComponent extends Component {
  override def name(): String = "velox-delta24-write"
  ...
  override def injectRules(injector: Injector): Unit = {
    ...
    legacy.injectTransform {
      c =>
        val offload = Seq(OffloadDeltaDmlScan(), OffloadDeltaCommand()).map(_.toStrcitRule())
