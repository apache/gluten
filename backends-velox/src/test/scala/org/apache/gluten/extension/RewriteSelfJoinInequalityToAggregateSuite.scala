...
import org.apache.spark.sql.catalyst.expressions.{Alias, GreaterThan, InSubquery, Literal, Not}
...
class RewriteSelfJoinInequalityToAggregateSuite extends WholeStageTransformerSuite {
  ...
  test("Nested self-join without wrapper/top-level Project fails closed: A2 arity guard") {
    // ReorderJoin excluded so the bare nested self-join reaches the arity guard; ka/va vs kb/vb
    // renames keep `SELECT *` names distinct (plain `T s1 JOIN T s2` emits two `k`).
    setupTable()
    spark.sql(
      """CREATE OR REPLACE TEMP VIEW D AS SELECT * FROM VALUES
        |  (1), (3), (6) AS D(k)""".stripMargin)
    ...
    withSQLConf(SQLConf.OPTIMIZER_EXCLUDED_RULES.key -> ReorderJoin.ruleName) {
      // Assert a self-join child (sameResult + inequality) survives, proving the guard is reached.
      def isTargetSelfJoin(p: LogicalPlan): Boolean = p match {
        case j: Join if j.joinType == Inner =>
          j.left.sameResult(j.right) &&
          j.condition.exists(_.exists {
            case _: Not => true
            case _ => false
          })
        case _ => false
