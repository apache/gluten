...
class VeloxIcebergDeletionVectorSuite extends IcebergSuite {
  ...
  test("read Spark deletion vector and union it in a native delete") {
    withTable("iceberg_dv_delete") {
      ...
      assert(getExecutedPlan(sparkDvRead).collect {
      ...
      checkAnswer(
        spark.sql("""
                    ...
                    |FROM default.iceberg_dv_delete.delete_files
                    |""".stripMargin),
        Seq(Row("PUFFIN", 2L))
      )
    }
    ...
    withTable("iceberg_dv_mutation") {
      ...
      assert(
        spark
          .sql("""
                 |SELECT count(*)
                 |FROM default.iceberg_dv_mutation.delete_files
  ...
  test("native deletion vectors preserve partitions and filter split ranges") {
    withTable("iceberg_dv_partitioned") {
      ...
      withSQLConf("spark.sql.files.maxPartitionBytes" -> "256") {
        ...
        val scan = getExecutedPlan(splitRead).collectFirst {
      ...
      checkAnswer(
        spark.sql("""
                    |SELECT count(*)
                    |FROM default.iceberg_dv_partitioned.delete_files
