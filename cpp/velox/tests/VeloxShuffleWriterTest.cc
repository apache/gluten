...
class VeloxShuffleWriterTest : public ::testing::TestWithParam<ShuffleTestParams>, public VeloxShuffleWriterTestBase {
 protected:
  ...
  void getRowVectors(
    ...
    const auto iter =
        reader->read(std::make_shared<TestStreamReader>(std::move(in)), ShuffleReader::OutputType::kRowVector, 0);
    while (iter->hasNext()) {
...
class VeloxShuffleReaderStreamMergeTest : public ::testing::Test, public VeloxShuffleWriterTestBase {
 protected:
  ...
  std::vector<RowVectorPtr> readStreams(
    ...
    const auto iter =
        reader->read(std::make_shared<MultiStreamReader>(std::move(streams)), ShuffleReader::OutputType::kRowVector, 0);
