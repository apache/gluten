...
std::string DeltaUuidUtils::encodeUuidToZ85(const Uuid& uuid) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    encodeZ85Block(uuid.data() + (i * 4), &result[i * 5]);
  }
...
std::string DeltaUuidUtils::encodeBytesToBase85(std::string_view data) {
  ...
  for (size_t blockIndex = 0; blockIndex < paddedSize / 4; ++blockIndex) {
    ...
    const size_t bytesToCopy = inputOffset < data.size() ? std::min<size_t>(4, data.size() - inputOffset) : 0;
    if (bytesToCopy > 0) {
...
DeltaUuidUtils::Uuid DeltaUuidUtils::decodeZ85ToUuid(const std::string& z85) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    decodeZ85Block(z85.data() + (i * 5), uuid.data() + (i * 4));
  }
...
std::string DeltaUuidUtils::decodeBase85ToBytes(std::string_view encoded, size_t decodedSize) {
  ...
  for (size_t blockIndex = 0; blockIndex < encoded.size() / 5; ++blockIndex) {
    decodeZ85Block(encoded.data() + (blockIndex * 5), reinterpret_cast<uint8_t*>(decoded.data() + (blockIndex * 4)));
  }
  decoded.resize(decodedSize);
  return decoded;
}

void DeltaUuidUtils::encodeZ85Block(const uint8_t* input, char* output) {
  // Convert 4 bytes to a 32-bit value (big-endian)
  uint32_t value = (static_cast<uint32_t>(input[0]) << 24) | (static_cast<uint32_t>(input[1]) << 16) |
...
void DeltaUuidUtils::decodeZ85Block(const char* input, uint8_t* output) {
...
std::pair<std::string, DeltaUuidUtils::Uuid> DeltaUuidUtils::extractUuidFromZ85(const std::string& z85) {
