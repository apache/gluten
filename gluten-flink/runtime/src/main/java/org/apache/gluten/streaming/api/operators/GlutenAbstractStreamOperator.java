...
public abstract class GlutenAbstractStreamOperator<OUT>
  ...
  protected static final Logger LOG = LoggerFactory.getLogger(GlutenAbstractStreamOperator.class);
  ...
  /**
   * {@code KeySelector} for extracting a key from an element being processed. This is used to scope
   * keyed state to a key. This is null if the operator is not a keyed operator.
   *
   ...
   * {@link GlutenAbstractStreamOperator} may attempt to read from it as well to restore heap-based
   * timers and ultimately fail with read errors. By setting this flag to {@code true}, this allows
   * the {@link GlutenAbstractStreamOperator} to know that the data written in the raw keyed states
   * were not written by the timer services, and skips the timer restore attempt.
   *
  ...
  /**
   * Stream operators with state, which want to participate in a snapshot need to override this hook
   * method.
   *
  ...
  /**
   * Return the operator name. If the runtime context has been set, then the task name with subtask
  ...
  /**
   * Returns a context that allows the operator to query information about the execution and also to
   * interact with systems such as broadcast variables and managed state. This also allows to
   * register timers.
   */
  @VisibleForTesting
   ...
   * when requesting them, if you call this method with the same key multiple times you will get the
   ...
   *
   * @param name The name of the requested timer service. If no service exists under the given name
  ...
  private void processWatermarkStatus(WatermarkStatus watermarkStatus, int index) throws Exception {
  ...
  public void processRecordAttributes1(RecordAttributes recordAttributes) {
    ...
    output.emitRecordAttributes(
        new RecordAttributesBuilder(Arrays.asList(lastRecordAttributes1, lastRecordAttributes2))
            .build());
  }
