 ...
 * accordingly.
 *
 * Note: String constants that contain an ExprId-like fragment (e.g., Brand#23 in TPCH q19) are
 * shielded from ExprId normalization, so golden files are stable no matter whether the suites run
 * together in one JVM or in isolation.
 */
trait GlutenPlanStabilityTestTrait {
  ...
  override protected def testQuery(tpcdsGroup: String, query: String, suffix: String = ""): Unit = {
    ...
    withSQLConf(
      ...
      val explain = glutenNormalizeLocation(
        glutenNormalizeIds(qe.explainString(FormattedMode), getHashLiterals(queryString)))
...
private[sql] object GlutenPlanStabilityTestTrait {
  ...
  // Matches single-quoted SQL string literals containing an ExprId-like "#<digits>" fragment,
  // e.g. 'Brand#12' in TPCH q19 or 'scholaramalgamalg #14' in TPCDS q53.
  private val hashLiteralRegex = "'([^']*#\\d+[^']*)'".r
  // Used to temporarily mask '#' inside such literals; never occurs in explain output.
  private val hashMask = '\u0000'

  /**
   * Extracts string literals that contain an ExprId-like "#<digits>" fragment from the query text.
   * Such literals appear verbatim in the explain output, where they are syntactically
   * indistinguishable from attribute ExprIds. Longer literals are returned first so that a literal
   * that is a prefix of another one does not shadow it during masking.
   */
  def getHashLiterals(queryString: String): Seq[String] = {
    hashLiteralRegex.findAllMatchIn(queryString).map(_.group(1)).toSeq.distinct.sortBy(-_.length)
  }

  /**
   * Normalizes ExprIds, plan ids and Gluten's _pre_N expression names to sequential numbers in
   * encounter order, so that plans can be compared against golden files across JVM sessions.
   *
   * Occurrences of `hashLiterals` (string constants from the query, e.g. Brand#12 in TPCH q19) are
   * shielded from ExprId normalization. Without this, a literal like Brand#12 shares the "#12"
   * token with whichever attribute happens to be allocated ExprId 12, so the normalized output
   * would depend on the JVM's ExprId counter -- stable when a whole module runs in one JVM, but
   * different in an isolated suite run.
   */
  def glutenNormalizeIds(plan: String, hashLiterals: Seq[String] = Nil): String = {
    val maskedPlan = hashLiterals.foldLeft(plan) {
      (p, literal) => p.replace(literal, literal.replace('#', hashMask))
    }
    ...
    val preExprNormalized = preExprIdRegex.replaceAllIn(
      planIdNormalized,
      m => preExprMap.getOrElseUpdate(m.toString(), s"${m.group("prefix")}${preExprMap.size + 1}"))

    preExprNormalized.replace(hashMask, '#')
  }
}
