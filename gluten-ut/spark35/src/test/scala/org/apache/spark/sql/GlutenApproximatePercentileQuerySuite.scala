...
class GlutenApproximatePercentileQuerySuite
  ...
  private def assertApproxEqual(actual: Any, expected: Double, tolerance: Double): Unit = {
    val actualDouble = actual match {
      ...
      case ldt: LocalDateTime => DateTimeUtils.localDateTimeToMicros(ldt).toDouble
      case other =>
  ...
  // Override: different column types all have the same boundary difference.
  testGluten("percentile_approx, different column types") {
    withTempView(ptable) {
      val intSeq = 1 to 1000
      val data: Seq[(java.math.BigDecimal, Date, Timestamp, LocalDateTime)] = intSeq.map {
        i =>
          (
      ...
      data.toDF("cdecimal", "cdate", "ctimestamp", "ctimestampntz").createOrReplaceTempView(ptable)
      val result = spark
        .sql(s"""SELECT
      ...
      assertApproxSeqEqual(result.get(3), Seq(250.0, 500.0, 750.0), kllTolerance)
    }
  }

  // Override: KLL error bound formula differs from GK's floor(N/accuracy).
  // Spark's original assertion: error <= floor(N/accuracy), which for accuracy=10000
  // requires exact match (floor(1000/10000) = 0). KLL may differ by 1-2 even with
  // high accuracy due to discrete rounding at percentile boundaries.
    ...
    withTempView(ptable) {
      ...
      for (accuracy <- accuracies) {
        for (expectedPercentile <- expectedPercentiles) {
          ...
          // KLL error bound: max(floor(N/accuracy), kllTolerance).
          // The kllTolerance accounts for boundary rounding in discrete data.
          val maxError =
